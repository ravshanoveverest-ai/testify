...
export default function ExamRoom() {
  ...
  if (step === "result" && result) {
    ...
    // UIcons uchun classlar tanlash
    let iconClass = "fi fi-rr-party-horn text-green-500"; 
    let bg = "bg-green-50";
    let color = "text-green-500";
    
    if (percentage < 60) { 
      iconClass = "fi fi-rr-hand-holding-heart text-blue-500"; 
      color = "text-blue-500"; 
      bg = "bg-blue-50"; 
    }
    if (percentage < 40) { 
      iconClass = "fi fi-rr-sad-tear text-red-500"; 
      color = "text-red-500"; 
      bg = "bg-red-50"; 
    }
    ...
    return (
      <div className="min-h-[85vh] flex items-center justify-center p-4 relative overflow-hidden">
        ...
        <div className="bg-white p-10 rounded-[2.5rem] shadow-2xl border border-gray-100 max-w-md w-full text-center animate-pop z-10 relative">
          ...
          <Link href="/">
            <button className="w-full py-4 bg-gray-900 text-white font-black rounded-xl hover:bg-black transition-all shadow-lg active:scale-95 flex items-center justify-center gap-2">
  ...
  if (step === "exam" && examData) {
    ...
    return (
      <div className="max-w-4xl mx-auto p-4 sm:p-6 lg:p-8 mt-4 animate-pop">
        <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100 flex flex-col md:flex-row items-center justify-between gap-4 mb-8 sticky top-24 z-30">
          <div>
            <span className={`px-3 py-1.5 rounded-lg text-xs font-black uppercase tracking-wider mb-2 inline-flex items-center gap-1.5 ${isWritten ? 'bg-orange-100 text-orange-700' : 'bg-purple-100 text-purple-700'}`}>
              {isWritten ? (
                <><i className="fi fi-rr-edit"></i> Yozma Nazorat</>
              ) : (
                <><i className="fi fi-rr-list-check"></i> Test Nazorati</>
              )}
            </span>
            <h1 className="text-2xl font-black text-gray-900">{examData.title}</h1>
            <p className="text-gray-500 font-bold text-sm mt-1 flex items-center gap-2">
              <i className="fi fi-rr-user text-gray-400"></i> O'quvchi: <span className="text-blue-600">{name}</span>
            </p>
          </div>
          
          <div className="flex items-center gap-4 bg-gray-50 px-6 py-3 rounded-2xl border border-gray-100">
             <div className="text-center bg-red-50 px-4 py-2 rounded-xl border border-red-100">
                <p className={`text-2xl font-black flex items-center justify-center gap-2 ${timeLeft < 60000 ? "text-red-600 animate-pulse" : "text-gray-800"}`}>
                  <i className="fi fi-rr-clock-three text-lg"></i> {formatTime(timeLeft)}
                </p>
                <p className="text-[10px] text-red-500 font-bold uppercase mt-1">Qolgan vaqt</p>
             </div>
             <div className="w-px h-8 bg-gray-200"></div>
             <div className="text-center px-2">
                <p className="text-2xl font-black text-blue-600 flex items-center justify-center gap-2">
                  <i className="fi fi-rr-checkbox text-lg"></i> {Object.keys(answers).length} <span className="text-gray-400 text-lg">/ {examData.questions.length}</span>
                </p>
                <p className="text-[10px] text-blue-500 font-bold uppercase mt-1">Belgilandi</p>
             </div>
          </div>
        </div>
        ...
        <div className="space-y-8 mb-12">
          {examData.questions.map((q, index) => (
            <div key={q._id} className="bg-white p-6 sm:p-8 rounded-[2rem] shadow-sm border border-gray-100">
              <h3 className="text-lg font-bold text-gray-900 mb-6 flex gap-3 leading-relaxed">
                <span className={`w-8 h-8 flex-shrink-0 flex items-center justify-center rounded-full ${isWritten ? 'bg-orange-50 text-orange-600' : 'bg-purple-50 text-purple-600'}`}>
                  {index + 1}
                </span> 
                {q.text}
              </h3>
                    ...
                    {q.options.map((option, optIdx) => {
                      ...
                      return (
                        <button 
                          ...
                          className={`text-left p-4 rounded-xl border-2 font-bold text-sm transition-all flex items-start gap-3 group outline-none ${isSelected ? "border-purple-600 bg-purple-50 text-purple-800 shadow-sm" : "border-gray-100 bg-white text-gray-600 hover:border-purple-200"}`}
                        >
                          <div className={`w-5 h-5 rounded-full border-2 flex-shrink-0 flex items-center justify-center mt-0.5 transition-colors ${isSelected ? "border-purple-600 bg-purple-600" : "border-gray-300 bg-white"}`}>
                            {isSelected && <i className="fi fi-rr-check text-white text-[10px] translate-y-[1px]"></i>}
                          </div>
                          <span>{option}</span>
                        </button>
                      );
        ...
        <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100 text-center">
          <button onClick={() => handleSubmitExam(false)} disabled={isLoading} className="px-12 py-4 bg-gradient-to-r from-green-500 to-emerald-600 text-white font-black text-lg rounded-xl transition-all shadow-lg hover:shadow-green-500/30 active:scale-95 disabled:opacity-70 flex items-center justify-center gap-3 mx-auto">
            {isLoading ? (
              <><i className="fi fi-rr-spinner animate-spin"></i> Yuborilmoqda...</>
            ) : (
              <><i className="fi fi-rr-flag"></i> Imtihonni Yakunlash</>
            )}
          </button>
        </div>
      </div>
  ...
  return (
    <div className="min-h-[85vh] flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="bg-white/80 backdrop-blur-xl p-8 sm:p-12 rounded-[2.5rem] shadow-2xl border border-white max-w-md w-full animate-pop z-10">
        <div className="text-center mb-8">
          <div className="w-20 h-20 bg-gradient-to-br from-purple-600 to-blue-600 text-white rounded-[1.5rem] flex items-center justify-center text-4xl mx-auto mb-6 rotate-3">
            <i className="fi fi-rr-target"></i>
          </div>
          <h1 className="text-3xl font-black text-gray-900 mb-2">Imtihonga kirish</h1>
          <p className="text-gray-500 font-medium">Ustozingiz bergan kodni kiriting</p>
        </div>
        ...
        {error && (
          <div className="bg-red-50 text-red-600 p-4 rounded-xl mb-6 text-sm font-bold flex justify-center items-center gap-2">
        ...
        <form onSubmit={handleJoinExam} className="space-y-6">
          <div className="relative">
            ...
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full pl-11 pr-5 py-4 rounded-xl border-2 border-gray-100 font-bold outline-none focus:border-purple-500 transition-all text-lg text-gray-800" placeholder="Ism Familiyangiz" />
          </div>
          ...
          <div className="relative">
            ...
            <input type="text" value={passcode} onChange={(e) => setPasscode(e.target.value.toUpperCase())} className="w-full pl-11 pr-5 py-4 rounded-xl border-2 border-gray-100 font-black tracking-widest uppercase text-center outline-none focus:border-purple-500 transition-all text-xl text-gray-800" placeholder="TEST-XXXXX" maxLength={15}/>
          </div>
          
          <button type="submit" disabled={isLoading} className="w-full py-4 bg-gradient-to-r from-purple-600 to-blue-600 text-white font-black text-lg rounded-xl transition-all shadow-lg active:scale-95 disabled:opacity-50 flex justify-center items-center gap-2">
            {isLoading ? (
              <><i className="fi fi-rr-spinner animate-spin"></i> Tekshirilmoqda...</>
            ) : (
              <><i className="fi fi-rr-rocket-lunch"></i> Kirish</>
            )}
          </button>
        </form>
      </div>
