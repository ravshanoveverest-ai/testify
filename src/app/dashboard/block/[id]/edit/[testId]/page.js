...
export default function EditTest() {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setSuccessMsg("Test muvaffaqiyatli yangilandi! ✏️"); 
      }
    } catch (error) {
      console.log(error);
    }
  ...
  if (isLoading) return <div className="p-20 text-center font-bold text-gray-500 flex justify-center items-center gap-2"><span className="animate-spin text-xl">⏳</span> Test yuklanmoqda...</div>;
        ...
        <form className="space-y-6">
          ...
          <div>
            ...
            <textarea 
              ...
              className="w-full px-5 py-4 rounded-xl border border-gray-300 focus:ring-2 focus:ring-blue-500 outline-none transition-all font-mono text-sm bg-gray-50 focus:bg-white leading-relaxed custom-scrollbar placeholder:text-gray-400/80"
              placeholder={`[JSON Formati]:\n[\n  {\n    "question": "O'zbekiston poytaxti?",\n    "options": ["Toshkent", "Samarqand"],\n    "answer": "Toshkent"\n  }\n]\n\nYOKI [TXT Formati]:\nQ: O'zbekiston poytaxti?\nA) Toshkent\nB) Samarqand\nANSWER: A`}
            ></textarea>
          </div>

          <button type="button" onClick={handleUpdate} className="w-full bg-orange-500 text-white font-extrabold py-4 rounded-xl hover:bg-orange-600 transition-all shadow-md text-lg active:scale-95">
            O'zgarishlarni Saqlash 💾
          </button>
        </form>
      </div>
      ...
      {successMsg && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl text-center animate-pop">
            ...
            <h3 className="text-xl font-black text-gray-900 mb-3">Ajoyib!</h3>
            <p className="text-gray-600 mb-8 font-medium">{successMsg}</p>
            <button onClick={handleCloseSuccess} className="w-full py-4 bg-gray-900 text-white font-bold rounded-xl hover:bg-black transition-colors shadow-lg active:scale-95">
              Davom etish
            </button>
          </div>
        </div>
      ...
      {errorMsg && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-3xl p-8 max-w-sm w-full shadow-2xl text-center animate-pop">
            ...
            <h3 className="text-xl font-black text-gray-900 mb-3">Xatolik yuz berdi!</h3>
            <p className="text-gray-600 mb-8 font-medium">{errorMsg}</p>
            <button onClick={() => setErrorMsg("")} className="w-full py-4 bg-gray-900 text-white font-bold rounded-xl hover:bg-black transition-colors shadow-lg active:scale-95">
              Tushunarli
            </button>
          </div>
        </div>
