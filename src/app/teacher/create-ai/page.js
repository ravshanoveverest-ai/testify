...
export default function CreateAIExam() {
  ...
  const [rawText, setRawText] = useState("");
  const [examType, setExamType] = useState("test"); // 'test' yoki 'written'
  const [creationMode, setCreationMode] = useState("ai"); // YANGI: 'ai' yoki 'manual'
  const [maxScore, setMaxScore] = useState(100);
    ...
    try {
      const res = await fetch("/api/teacher/exam", {
        ...
        body: JSON.stringify({
          ...
          text: rawText,
          examType,
          creationMode, // YANGI: Qaysi rejimdaligini jo'natamiz
          maxScore: Number(maxScore),
          totalQuestions: Number(totalQuestions),
          questionsPerStudent: Number(questionsPerStudent),
          studentCount: Number(studentCount),
  ...
  const downloadWordDoc = () => {
    ...
    successData.questions.forEach((q, i) => {
      ...
      if (successData.type === "test" && q.options && q.options.length > 0) {
        content += `<ul style="list-style-type: none; padding-left: 10px;">`;
        q.options.forEach(opt => {
          content += `<li style="margin-bottom: 5px;">&#9711; ${opt}</li>`;
        });
      ...
      content += `<div style="background-color: #f0fdf4; padding: 10px; border-left: 4px solid #16a34a; margin-top: 10px;">`;
      content += `<p style="margin:0; color: #166534;"><b>To'g'ri Javob:</b> ${q.answer || 'Kiritilmagan'}</p>`;
      content += `</div></div>`;
    });
  ...
  // MUVAFFAQIYATLI YARATILGANDA
  if (successData) {
    return (
      <div className="max-w-3xl mx-auto p-4 sm:p-6 lg:p-8 mt-10 animate-pop text-center">
        <div className="bg-white rounded-[2.5rem] p-10 border border-gray-100 shadow-2xl">
          <div className="w-24 h-24 bg-green-100 text-green-600 rounded-full flex items-center justify-center text-4xl mx-auto mb-6 shadow-inner">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <button onClick={downloadWordDoc} className="px-8 py-4 bg-blue-600 hover:bg-blue-700 text-white font-black rounded-xl shadow-lg hover:shadow-blue-500/30 transition-all flex items-center justify-center gap-2">
              <i className="fi fi-rr-document"></i> Word formatida yuklab olish
            </button>
            <Link href="/teacher/rooms">
              <button className="px-8 py-4 bg-gray-900 hover:bg-black text-white font-black rounded-xl shadow-lg transition-all w-full sm:w-auto flex items-center justify-center gap-2">
                Parollarni ko'rish <i className="fi fi-rr-arrow-right"></i>
              </button>
            </Link>
          </div>
        </div>
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 sm:p-6 lg:p-8 mt-4 animate-pop">
      <div className="flex items-center gap-4 mb-8">
        <Link href="/teacher" className="w-10 h-10 bg-white rounded-full flex items-center justify-center font-bold text-gray-500 shadow-sm hover:bg-gray-50 hover:text-gray-700 transition-colors">
          <i className="fi fi-rr-arrow-left translate-y-0.5"></i>
        </Link>
        <div>
          <h1 className="text-3xl font-black text-gray-900 flex items-center gap-3">
            <i className="fi fi-rr-magic-wand text-purple-600"></i> AI Imtihon Yaratish
          </h1>
        </div>
      </div>
      ...
      <form onSubmit={handleGenerateExam} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white p-6 sm:p-8 rounded-[2rem] border border-gray-100 shadow-sm">
            <h2 className="text-xl font-bold text-gray-800 mb-6 flex items-center gap-2">
              <i className="fi fi-rr-info text-gray-500"></i> 1. Asosiy ma'lumotlar
            </h2>
            {error && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">Nazorat turi</label>
                <div className="flex gap-2 bg-gray-50 p-1.5 rounded-xl border border-gray-200">
                  <button type="button" onClick={() => setExamType("test")} className={`flex-1 py-3 rounded-lg font-bold text-sm transition-all flex items-center justify-center gap-2 ${examType === "test" ? "bg-white text-purple-600 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}>
                    <i className="fi fi-rr-list-check"></i> Test
                  </button>
                  <button type="button" onClick={() => setExamType("written")} className={`flex-1 py-3 rounded-lg font-bold text-sm transition-all flex items-center justify-center gap-2 ${examType === "written" ? "bg-white text-purple-600 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}>
                    <i className="fi fi-rr-edit"></i> Yozma ish
                  </button>
                </div>
              </div>
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="number" min="1" value={maxScore} onChange={(e) => setMaxScore(e.target.value)} className="w-full pl-10 pr-5 py-3.5 rounded-xl border border-gray-200 bg-gray-50 font-bold outline-none focus:ring-2 focus:ring-purple-500" placeholder="100"/>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">Boshlanish</label>
                <input type="datetime-local" value={startTime} onChange={(e) => setStartTime(e.target.value)} className="w-full px-5 py-4 rounded-xl border border-gray-200 bg-gray-50 font-bold text-gray-700 outline-none focus:ring-2 focus:ring-purple-500"/>
              </div>
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">Tugash</label>
                <input type="datetime-local" value={endTime} onChange={(e) => setEndTime(e.target.value)} className="w-full px-5 py-4 rounded-xl border border-gray-200 bg-gray-50 font-bold text-gray-700 outline-none focus:ring-2 focus:ring-purple-500"/>
              </div>
            </div>

            <div className="mt-8 border-t border-gray-100 pt-6">
              <label className="block text-sm font-bold text-gray-700 mb-2">Qanday usulda tahlil qilinadi?</label>
              <div className="flex gap-2 bg-gray-50 p-1.5 rounded-xl border border-gray-200 mb-4">
                <button type="button" onClick={() => setCreationMode("ai")} className={`flex-1 flex justify-center items-center gap-2 py-3 rounded-lg font-bold text-sm transition-all ${creationMode === "ai" ? "bg-white text-gray-900 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}>
                  <i className="fi fi-rr-sparkles"></i> AI Analiz (Aqlli)
                </button>
                <button type="button" onClick={() => setCreationMode("manual")} className={`flex-1 flex justify-center items-center gap-2 py-3 rounded-lg font-bold text-sm transition-all ${creationMode === "manual" ? "bg-white text-gray-900 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}>
                  <i className="fi fi-rr-keyboard"></i> Qo'lda (TXT/JSON)
                </button>
              </div>

              <label className="block text-sm font-bold text-gray-700 mb-2">
                {creationMode === "ai" ? "AI tahlil qilishi uchun matn" : "Tayyor TXT yoki JSON formatda kiriting (Tezkor):"}
              </label>
              <textarea 
                rows="8" 
                value={rawText} 
                onChange={(e) => setRawText(e.target.value)} 
                placeholder={creationMode === "ai" ? "Matnni tashlang..." : "1. O'zbekiston poytaxti?\nA) Toshkent\nB) Samarqand\nJavob: A\n\nYoki JSON formatida kiriting..."} 
                className="w-full px-5 py-4 rounded-xl border border-gray-200 bg-gray-50 font-medium outline-none focus:ring-2 focus:ring-purple-500 resize-y text-gray-700 placeholder-gray-400"
              ></textarea>
            </div>
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 sm:p-8 rounded-[2rem] border border-gray-100 shadow-sm sticky top-24">
            <h2 className="text-xl font-bold text-gray-800 mb-6 flex items-center gap-2">
              <i className="fi fi-rr-settings-sliders text-gray-500"></i> 2. Sozlamalar
            </h2>
            <div className="space-y-5 mb-8">
              <div>
                <label className="flex justify-between text-sm font-bold text-gray-700 mb-2">
                  <span>{creationMode === 'ai' ? 'Jami AI tuzadigan savollar' : 'Jami bazadagi savollar'}</span>
                </label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                    <i className="fi fi-rr-layer-plus text-gray-400"></i>
                  </div>
                  <input type="number" min="1" value={totalQuestions} onChange={(e) => setTotalQuestions(e.target.value)} className="w-full pl-10 pr-4 py-3 rounded-xl border border-gray-200 bg-gray-50 font-bold outline-none focus:ring-2 focus:ring-purple-500 text-gray-700"/>
                </div>
              </div>
              
              <div>
                <label className="flex justify-between text-sm font-bold text-gray-700 mb-2">
                  <span>O'quvchiga tushadigani</span>
                </label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                    <i className="fi fi-rr-user text-gray-400"></i>
                  </div>
                  <input type="number" min="1" value={questionsPerStudent} onChange={(e) => setQuestionsPerStudent(e.target.value)} className="w-full pl-10 pr-4 py-3 rounded-xl border border-gray-200 bg-gray-50 font-bold outline-none focus:ring-2 focus:ring-blue-500 text-gray-700"/>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                    <i className="fi fi-rr-key text-gray-400"></i>
                  </div>
                  <input type="number" min="1" value={studentCount} onChange={(e) => setStudentCount(e.target.value)} className="w-full pl-10 pr-4 py-3 rounded-xl border border-gray-200 bg-gray-50 font-bold outline-none focus:ring-2 focus:ring-green-500 text-gray-700"/>
                </div>
              </div>
            </div>
            
            <button type="submit" disabled={isLoading} className="w-full bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-black py-4 rounded-xl shadow-lg hover:shadow-purple-500/30 active:scale-95 disabled:opacity-70 flex justify-center items-center gap-2 transition-all">
              {isLoading ? (
                <>
                  <i className="fi fi-rr-spinner animate-spin"></i> Generatsiya...
                </>
              ) : (
                <>
                  <i className="fi fi-rr-magic-wand"></i> Yaratishni boshlash
                </>
              )}
            </button>
          </div>
